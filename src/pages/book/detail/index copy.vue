...
function onSearchResultChange(dom: any) {
  const width = wrapperRef.value?.clientWidth - configs.value.gap
...
function initPage() {
  nextTick(() => {
    ...
    let progress = 0
    if (storageBookInfo?.update_time && dayjs(storageBookInfo?.update_time) > dayjs(book.value?.update_time)) {
      progress = storageBookInfo.progress
    }
    else {
      progress = book.value?.progress
    }
    current.value = Math.min(Math.max(1, Math.round((progress ?? 0) * total.value)), total.value)
...
//   record(route.query.id as string)
// }, 1000)
...
//       interval = null // 可选，根据你的需求决定是否需要重置 interval
//     }
//   }
// }
...
watch(() => current.value, () => {
  if (total.value) {
    localStorage.setItem(`MM_READ_BOOK_${book_id.value}`, JSON.stringify({ progress: current.value / total.value, update_time: dayjs().format('YYYY-MM-DD HH:mm:ss') }))
    onUpdateBook()
  }
})
...
<template>
  ...
  >
    ...
    <BottomPop
      ...
      @search-result-change="onSearchResultChange"
    />
  </div>
</template>
